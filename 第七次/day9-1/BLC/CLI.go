...
func (cli *CLI) Yxh_Run()  {
	...
	if sendBlockCmd.Parsed() {
		...
		cli.yxh_send(from,to,amount,nodeID,*flagMine)
	}
	...
	if resetUTXOCMD.Parsed() {
		...
		cli.yxh_resetUTXOSet(nodeID)
	}

	if addresslistsCmd.Parsed() {

		//fmt.Println("输出所有区块的数据........")
		cli.yxh_addressLists(nodeID)
	}
	...
	if createBlockchainCmd.Parsed() {
		...
		cli.yxh_createGenesisBlockchain(*flagCreateBlockchainWithAddress,nodeID)
	}
	...
	if getbalanceCmd.Parsed() {
		...
		cli.yxh_getBalance(*getbalanceWithAdress,nodeID)
	}

	if startNodeCmd.Parsed() {


		cli.yxh_startNode(nodeID,*flagMiner)
	}
