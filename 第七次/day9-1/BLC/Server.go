...
func yxh_startServer(nodeID string,minerAdd string)  {

	// 当前节点的IP地址
	yxh_nodeAddress = fmt.Sprintf("localhost:%s",nodeID)

	ln,err := net.Listen(PROTOCOL,yxh_nodeAddress)
	...
	bc := Yxh_BlockchainObject(nodeID)


	// 第一个终端：端口为3000,启动的就是主节点
	// 第二个终端：端口为3001，钱包节点
	// 第三个终端：端口号为3002，矿工节点
	if yxh_nodeAddress != yxh_knowNodes[0]{
		 // 此节点是钱包节点或者矿工节点，需要向主节点发送请求同步数据

		 yxh_sendVersion(yxh_knowNodes[0],bc)
	}
