...
//main.exe send -from [\"yxh\"] -to [\"yxh1\"] -amount [\"1\"]
//main.exe send -from [\"yxh\",\"yxh1\"] -to [\"yxh1\",\"yxh2\"] -amount [\"4\",\"1\"]
func printUsage()  {
...
func (cli *CLI) Run()  {
	...
	if sendBlockCmd.Parsed() {
		...
		for _, fromAdderss := range from {
			if IsValidForAdress([]byte(fromAdderss)) == false {
				printUsage()
				os.Exit(1)
			}
		}
		for _, toAdderss := range from {
			if IsValidForAdress([]byte(toAdderss)) == false {
				printUsage()
				os.Exit(1)
			}
		}
