...
func (blc *Blockchain) Printchain() {
	...
	for {
		...
		for _,tx := range block.Txs {
			...
			fmt.Println("txhash:", hex.EncodeToString(tx.TxHash))
			fmt.Println("已花费的交易信息Vins")
			for _,in := range tx.Vins {
				fmt.Printf("TXHash:%s,Vout:%d,ScriptPubKey:%s\n", hex.EncodeToString(in.TxHash),in.Vout, in.ScriptPubKey)
			}
			fmt.Println("所有交易输出信息Vouts")
			for _,out := range tx.Vouts {
				fmt.Printf("Vout:%d,ScriptPubKey:%s\n", out.Value, out.ScriptPubKey)
			}
...
func CreateBlockchainWithGenesisBlock(address string) *Blockchain {
	...
	err = db.Update(func(tx *bolt.Tx) error {
		...
		// 创建数据库表 创世区块初始化里只有output交易，没有input
